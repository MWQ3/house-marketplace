...
function EditListing() {
    ...
    useEffect(() => {
      const fetchListing = async () => {
        ...
        if(docSnap.exists()) {
          ...
          toast.error('Oops, Couldn\'t fetch edit details')
          navigate('/')
        }
      }
    ...
    const handleSubmit = async (e) => {
        ...
        const storeImgs = async (image) => {
            return new Promise((resolve, reject) => {
                ...
                (error) => {
                    reject(error)
                    toast.error('Ops, Couldn\'t Upload the image')
                }, 
                () => {
        ...
        const updatedDoc = doc(db, 'listings', param.listingId)
        await updateDoc(updatedDoc, formDataCopy)
        

        setLoading(false)
        toast.success('listing has been edited')
        navigate(`/category/${updatedDoc.type}/${updatedDoc.id}`)
    }
    ...
    const handleMutate = (e) => {
        ...
        // handle boolean & text, number values
        if(!e.target.files) {
            setFormData((prevState) => ({
                ...prevState,
